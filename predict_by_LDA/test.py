...
def filter_data_by_time(data_from="2014-6-04 23:59:59",data_to="2014-6-07 23:59:59"):
	maker_list=['NEC','SONY','FUJITSU','DELL','TOSHIBA']
	data = get_all_data(maker_list)
	return data[(data_from < data.end_time) & (data.end_time < data_to)]

def predict_one(predict,test_data):
	true_price = test_data.current_price.values
	#predict_price = [predict_by_all(a_id) for a_id in test_data.auction_id.values]
	predict_price =[]
	for i in range(len(test_data.auction_id.values)):
		try:
			price = predict(test_data.auction_id.values[i])
			predict_price.append((i,price))
		except KeyError ,e:
			print e
			pass
...
def test2(predict_list):
	test_data=filter_data_by_time()
	for predict in predict_list:
		(prange,prate) = predict_one(predict,test_data)
		print predict
		print "prange is ",prange
		print "prate is ",prate




predict_list=[
	# predict_by_multi.PredictOne.predict,
	# predict_by_multi.PredictAll.predict,
	# predict_by_multi.predict_KN,
	# predict_by_multi.predict.predict_LR,
	# predict_by_multi.predict.predict_LR2
	PredictOne.predict,
	PredictAll.predict,
	predict.predict_LR,
	predict.predict_LR2,
